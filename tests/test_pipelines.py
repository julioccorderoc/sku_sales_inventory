...
class TestInventoryPipelineTransform:
    ...
    def test_zero_fill_all_skus_present_per_channel(self, inventory_df):
        """Every SKU in SKU_ORDER must appear for every processed channel."""
        result = InventoryPipeline().transform(inventory_df)
        processed_channels = set(inventory_df["channel"].unique())
    ...
    def test_output_count(self, inventory_df):
        """Total rows = len(SKU_ORDER) × number of channels in input."""
        result = InventoryPipeline().transform(inventory_df)
        n_channels = len(inventory_df["channel"].unique())
        assert len(result) == len(settings.SKU_ORDER) * n_channels
...
class TestSalesPipelineTransform:
    def _make_pipeline_with_df(self, df):
        """Helper: construct pipeline, inject bundle_rows, call transform."""
        pipeline = SalesPipeline()
        pipeline.bundle_rows = []   # normally set in extract(); inject empty here
